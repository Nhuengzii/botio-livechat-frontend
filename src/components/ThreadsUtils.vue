  ...
  <template v-else-if="mode == 'searching'">
    <div>
      <div class="flex items-center ">
        <div class="pl-8" @click="() => { uiStore.conversationsThreadMode = 'normal' } ">
          <font-awesome-icon :icon="['fas', 'arrow-left']" size="xl" />
        </div>
        <div class="flex items-center w-full pr-4 pl-5 mr-8">
          <input type="text" 
          class="ml-2 bg-[#D9D9D9]  border border-gray-300 text-gray-900  outline-none   block w-full pl-10 p-1   dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          :placeholder="querying ? 'ค้นหา' : ' ค้นหาจากชื่อ หรือ แท็ก'" v-model="query">
          <div class="  "
            :class="[querying ? '' : 'z-50']">
