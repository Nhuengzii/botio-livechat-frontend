    ...
    <div class="bg-[#EAEAEA]  mx-2 min-w-[320px] duration-500" :class="[query? 'h-32' :'h-40']" >
      <div class="bg-white h-36 my-4">
        <div v-if="mode == 'normal'" class="flex pt-5 ml-5 justify-between">
            <div class="flex">ข้อความทั้งหมด <div class=" px-2  bg-[#D9D9D9] rounded-full ml-3 font-bold text-base">+999</div>
            </div>
      ...
      <div class=" flex  justify-around py-[1rem] px-2" :class="query? '':'pl-5 '">
            <div v-if="query"  :class="[query? 'mt-1.5 ':'']">
              ...
              <div class="relative w-full mr-4 duration-500" :class="[query? 'mt-2':'mt-2']">
                ...
                <input type="text"   @click="{ query = true; uiStore.conversationsThreadMode='searching';}" :class="query? '':' rounded-full'" 
